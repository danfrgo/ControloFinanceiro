...
namespace ControloFinanceiro.DAL.Mapeamentos
{
    class MesMap : IEntityTypeConfiguration<Mes>
    {
        public void Configure(EntityTypeBuilder<Mes> builder)
        {
            ...
            builder.HasIndex(m => m.Nome).IsUnique(); // evita meses repetidos

            // um mes pode ter n despesas e as despesas só podem estar relacionadas a um mes
            builder.HasMany(m => m.Despesas).WithOne(c => c.Mes);

            // um mes pode ter n Ganhos e um ganho só podem estar relacionado a um mes
            builder.HasMany(m => m.Ganhos).WithOne(c => c.Mes);
